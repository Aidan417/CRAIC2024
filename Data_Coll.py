...
import numpy as np
import threading
from car_drive import car_drive
from Img_Handle import lane_hsv
...
def control_car_process():
    ...
    # ser = serial.Serial('/dev/car', 9600)
    stop_num = 0
    while True:
        fn = '/dev/input/js0'
        jsdev = open(fn, 'rb')
        # ser.write(hex_change(1500, 1500))
        car_drive(1500, 1500)
        ...
        while True:
            evbuf = jsdev.read(8)
            if evbuf:
                time, value, type_b, number = struct.unpack('IhBB', evbuf)
                if type_b == 1:
                    ...
                    if button:
                        if (button == "b" and value == True):
                            # print("START")
                            # ser.write(conversion(1, 0, 75))
                            speed = 1560
                            get_image = 1
                            stop_num = 0
                        if ((button == "tr" and value == True)):
                            # print("Stop")
                            # ser.write(conversion(0, 0, 75))
                            speed = 1500
                            get_image = 0
                            stop_num += 1
                            if stop_num == 2:
                ...
                if type_b == 2:
                    ...
                    if axis == "x":
                        fvalue = value / 32767
                        angle = int(1500 - (fvalue * 410))
                        if angle < 1100:
                            angle=1100

                car_drive(speed, angle)
